...
def integrate_system_param(config, n, path, verbose=False):
    raise ValueError("Parameter dependent CGLE not implemented yet!")
...
def main(config):
    ...
    if not os.path.exists(config["GENERAL"]["save_dir"]+'/dat'):
        ...
        if config["MODEL"].getboolean("use_param"):
            integrate_system_param(config["SYSTEM"], int(config["TRAINING"]["n_train"]) +
    ...
    network = utils.Network(config["MODEL"])
    ...
    model = utils.Model(dataloader_train, dataloader_test, network, config["TRAINING"],
                        path=config["GENERAL"]["save_dir"]+'/')
    ...
    progress_bar = tqdm.tqdm(range(0, int(config["TRAINING"]['epochs'])),
                             ...
                             leave=True, desc=utils.progress(0, 0))
    ...
    for epoch in progress_bar:
        ...
        progress_bar .set_description(utils.progress(train_loss, val_loss))
    ...
    if verbose:
        ...
        prediction = tests.test_integration(model, dataset_test, dataset_train.svd, 1000, 4000,
                                            path=config["GENERAL"]["save_dir"])
    ...
    prediction = model.integrate(dataset_test, dataset_train.svd, 0, data_unperturbed["T"])
    ...
    plt.savefig('plot_cgle.eps')
    plt.savefig('plot_cgle.pdf')
    plt.show()


if __name__ == "__main__":
    config = configparser.ConfigParser()
    config.read('cgle_dmaps.cfg')
    main(config)
